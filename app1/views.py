...
def register(request):
    ...
    if not counters:
        ...
        elif len(phoneNumber) < 12 or len(phoneNumber) > 12:
            messages.info(request, 'Enter a valid 12 digit phone number')
            return redirect('register')

        # else generating otp and sending it to customer via email and text message on phone number
        else:
            phoneNumber = '+' + phoneNumber  # using +254 by default, so it will only work for kenya number
            ...
            try:
                ...
                # E-MAIL
                rec_email = email
                message = "Dear {}, OTP to book your position in queue is {}. Do not share it with anyone.".format(name,
                                                                                                                   otpNum)
...
def otp(request, phoneNumber):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'app1/otp.html', {'phoneNumber': phoneNumber})
...
def view_queue(request):
    if not counters:
        ...
        try:
            ...
            return render(request, 'app1/view_queue.html', {'counter_num': d.counter, 'pos': d.pos})
        except:
            messages.info(request, 'Token number not found')
            return redirect('queue details')

    else:
        return render(request, 'app1/view_queue.html', {'counter_num': "", 'pos': ""})
    ...
    if request.user.is_authenticated and request.user.username != 'admin':
        ...
        if n <= 0:
            ...
            while max(counters.values()) > min(counters.values()) + 1:
                ...
                try:
                    data = userData.objects.filter(counter=large_counter,
                                                   pos__gt=smallest_queue + 2)  # __gt means greater than
                    for i in data:
                        i.pos -= 1
                        i.save()
                except Exception as e:
        ...
        try:
            ...
            return render(request, 'app1/employee.html', {'name': d.name, 'token': d.token, 'counter': n})
        except Exception as e:
            print(e)
            messages.info(request, 'The queue is empty. Press "next customer" button when a customer arrives.')
            return render(request, 'app1/employee.html', {'name': '', 'token': '', 'counter': n})
    else:
        return redirect('login')
    ...
    if request.user.is_authenticated and request.user.username != 'admin':
        if request.method == 'POST':
            ...
            try:
                ...
                print(e)
                return render(request, 'app1/selectCounter.html', {'availableCounters': availableCounters})
